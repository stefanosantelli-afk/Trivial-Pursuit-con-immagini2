...
const DIFF_MAP = {
  1: { label: "Facile", points: 0.5 },
  2: { label: "Medio", points: 1 },
  3: { label: "Difficile", points: 2.5 },
};
...
const fxImageEl = document.getElementById("fxImage");
...
function setStatus(text){ statusPill.textContent = `${text} · ${APP_VERSION}`; } · ${APP_VERSION}`; }
...
function showFxImage(src, ms=2000){
  if(!fxImageEl) return;
  fxImageEl.src = src;
  fxImageEl.classList.remove("hidden");
  setTimeout(()=>{
    fxImageEl.classList.add("hidden");
    fxImageEl.removeAttribute("src");
  }, ms);
}

function bray2s(){
  // raglio sintetico ~2s (fallback se non c’è voce)
  ensureAudio();
  ...
  o1.frequency.exponentialRampToValueAtTime(420, now+0.4);
  o1.frequency.exponentialRampToValueAtTime(180, now+1.6);

  o2.frequency.setValueAtTime(110, now);
  o2.frequency.exponentialRampToValueAtTime(220, now+0.5);
  o2.frequency.exponentialRampToValueAtTime(90, now+1.6);
...
function resetTurnUI(){
  ...
  if(fxImageEl){ fxImageEl.classList.add("hidden"); fxImageEl.removeAttribute("src"); }
...
function onAnswer(isCorrect, correctText){
  ...
  if(isCorrect){
    ...
    // FX: sbagliato su Facile → asino + raglio (2s)
    if(rolledDiff.label === "Facile"){
      showFxImage("./assets/asino.png", 2000);
      const ok = speakFX("I A!", {durationMs: 2000, rate: 0.7, pitch: 0.6, volume: 1});
      if(!ok) bray2s();
    }

    feedbackEl.innerHTML = `❌ Sbagliato. La risposta corretta era: “${correctText}”.`;
  }

  // sblocca il passaggio turno SOLO dopo risposta
